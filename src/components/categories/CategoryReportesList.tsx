...
interface CategoryReportesListProps {
  category: Category;
  onViewReporte?: (reporteId: string) => void;
}

export const CategoryReportesList: React.FC<CategoryReportesListProps> = ({ category, onViewReporte }) => {
  ...
  // Filtrar reportes que tienen esta categoría específica
  const reportesWithCategory = reportes.filter(reporte => reporte.categoria_id === category.id);
    ...
    <>
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FileText className="h-5 w-5" />
            Reportes con esta Categoría ({reportesWithCategory.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {reportesWithCategory.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <AlertCircle className="h-12 w-12 text-muted-foreground mb-4" />
              <p className="text-lg font-medium text-muted-foreground mb-2">
                No hay reportes con esta categoría
              </p>
              <p className="text-sm text-muted-foreground">
                Esta categoría no está asignada a ningún reporte actualmente
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {reportesWithCategory.map((reporte) => (
                <div 
                  key={reporte.id} 

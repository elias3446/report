...
export const ReporteDetail = ({ reporte, onEdit, onBack }: ReporteDetailProps) => {
  ...
  const handleMarkAsResolved = () => {
    markAsResolved(reporte.id);
  };

  const handleMarkAsPending = () => {
    markAsPending(reporte.id);
  };

  const handleActivateReporte = () => {
    activateReporte(reporte.id);
  };

  const handleDeactivateReporte = () => {
    deactivateReporte(reporte.id);
  };
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="pt-2 border-t">
                  <h5 className="text-sm font-medium mb-2">Visibilidad Pública</h5>
                  <div className="flex flex-wrap gap-2">
                    {reporte.activo === false && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={handleActivateReporte}
                    ...
                    {reporte.activo === true && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={handleDeactivateReporte}
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {reporte.activo === true ? 'Visible en reportes públicos' : 
                     reporte.activo === false ? 'Oculto del público' : 
                     'Los reportes resueltos no aparecen en público'}
                  </p>
                </div>
              </div>
